 ...
 *       random activity placement across replicas is correct — there is no in-memory
 *       {@code AgentRunContext}, blocked future, or {@code ThreadLocal}.</li>
 *   <li><b>Observability</b>: every step is a history entry; {@link AgentStatus} is published
 *       as custom status.</li>
 * </ul>
 *
 * <p><b>Milestone 1 scaffold.</b> Starting this workflow currently has to be done manually (e.g.
 * {@code DaprWorkflowClient.scheduleNewWorkflow("react-agent", input, id)}); wiring the
 * {@code @Agent} proxy entry point to start it (and rendering the prompt from method args) is
 * the next step. Long agents should periodically {@code ctx.continueAsNew(...)} to cap history.
 */
@ApplicationScoped
